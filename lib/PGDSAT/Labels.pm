...
#---------------------------
		...
		'2.2' => {
			...
			'description' => 'The permissions must be 0755 owned by root user and group.',
		},
		'2.3' => {
			'title' => 'Disable PostgreSQL Command History',
			'description' => 'Disabling the PostgreSQL command history reduces the probability of exposing
sensitive information, such as passwords, encryption keys, or sensitive data.',
		...
		'2.4' => {
			...
			'description' => 'One can set a password in a PostgreSQL connection service file readable for all users. Verify the password option is not used in a connection service file.',
		},
		'2.5' => {
...
#-------------------------------
		...
		'2.2' => {
			'title' => 'Vérifiez que le répertoire d\'extension possède les droits d\'accès et de propriété appropriés.',
			'description' => 'Les permissions doivent être 0755 et appartenir à l\'utilisateur et au groupe root.',
		},
		'2.3' => {
			'title' => 'Désactiver l\'historique des commandes PostgreSQL',
			'description' => 'La désactivation de l\'historique des commandes PostgreSQL réduit la probabilité d\'exposer des informations sensibles, telles que des mots de passe, des clés de chiffrement ou des données sensibles.',
		},
		'2.4' => {
			'title' => 'Vérifiez que les mots de passe ne sont pas stockés dans le fichier de service.',
			'description' => 'Il est possible de définir un mot de passe dans un fichier de service de connexion PostgreSQL accessible à tous les utilisateurs. Vérifiez que l\'option de mot de passe n\'est pas utilisée dans un fichier de service de connexion.',
		},
		'2.5' => {
...
#---------------------------
		...
		'2.2' => {
			'title' => '确保扩展目录具有适当的所有权和权限',
			'description' => '权限必须为 0755，且必须由 root 用户和组拥有。',
		},
		'2.3' => {
			'title' => '禁用 PostgreSQL 命令历史记录',
			'description' => '禁用 PostgreSQL 命令历史记录可以降低泄露敏感信息（例如密码、加密密钥或敏感数据）的可能性。',
		},
		'2.4' => {
			'title' => '确保密码未存储在服务文件中',
			'description' => '可以在 PostgreSQL 连接服务文件中设置密码，该文件对所有用户都可读。请确认连接服务文件中未使用密码选项。',
		},
		'2.5' => {
